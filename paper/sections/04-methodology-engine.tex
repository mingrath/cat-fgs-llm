...
\node[val, left=of conv] (dec) {$>0.39$\\point\\decision};
...
basis read off the unforked Roboflow source --- $264$ pain boxes against
$1{,}819$ no-pain boxes, a prevalence of $\approx 12.7\%$ and an imbalance of
$\approx 6.9{:}1$ --- we stress that these are \emph{projected pre-fork targets,
not measured counts}: \texttt{data/} is empty (the corpus is not yet forked),
so every figure here is read off the source metadata rather than tallied from a
local export. They are also \emph{box counts, not individual cats}, and the
live-annotation basis ($246/1{,}811$, $\approx 12.0\%$) is recorded as a
projected discrepancy to be reconciled once the fork exists rather than silently
collapsed. We address imbalance with data and
...
is the wrapper's primary reported artifact --- cited supporting plumbing, not a
headline contribution --- and is developed later. The $\geq 0.90$ floor
is justified against the $90.7\%$ reference sensitivity of the Feline Grimace
Scale validation \citep{evangelista2019}, not against prior automated work.
The specificity that this floor buys is reported with bootstrap confidence
intervals across the five cat-grouped folds, but only once an enlarged
independent vet anchor exists: at the budgeted sample the Gate-0 power
calculation flags the $0.39$-ratio sens/spec operating point as
\texttt{reportable:false} (n\textsubscript{pos}$=16$, Clopper--Pearson
half-width $\approx 0.25$; see the evaluation-protocol chapter and
Section~\ref{sec:eng-detector-firewall}), so any specificity-with-CI claim at the
operating point is conditional on that future anchor and is exploratory, not a
result, at the budgeted $n$. Throughout, augmented and oversampled copies never
...
We emphasise that none of these rates can be produced today: the engine's data
substrate is empty (the corpus is not yet forked), the source corpus carries
Roboflow binary pain/no-pain labels only and no native $0$--$10$ FGS labels, and
the independent per-AU vet anchor does not yet exist. These four constraints are
itemised in the limitations chapter, and the real-cat detector evaluation is
named there as future work.
...
representation for the heads is the class token; a pooled patch-token
representation (mean, or mean$+$std per the cache \texttt{patch\_mode} flag,
whose code default is \texttt{mean\_std}) is cached as additional columns so that
the class-token-versus-pooled-patch comparison is a configuration flag rather
than a re-cache.
...
\node[hard, left=14mm of thr] (sum) {sum 0--10\\$>0.39$};
...
schedule $R(T)=1-\tau\cdot\min((T/T_k)^c,1)$ in its $c=1$ (linear-ramp)
specialization~\cite{han2018coteaching}: the keep-rate
...
reliability check --- is implemented as dataset-agnostic code under
\texttt{src/protocols/}, with thin adapters for action-unit override, column
mapping, and a generic non-FGS ordinal mode. A standalone test corpus exercises
the surface on synthetic arbitrary-AU data with no dependency on the torch engine
and is safe to run with the engine deleted, since it imports only the protocols
layer. That standalone corpus
(\texttt{src/protocols/standalone\_test\_corpus.py}) drives the
no-confound-at-this-power branch, asserting the null verdict on synthetic
zero-engine data, while a separate planted-positive recovery test
(\texttt{tests/test\_confound\_recovery.py}) asserts that a known
above-threshold capture-condition shift is flagged detected; together they
validate the headline confound leg's detection behaviour on planted positive
and planted negative controls before any real corpus exists. The engine itself remains conceded plumbing; the
evaluation-protocol chapter specifies the gate-enforced usage contract.

Two engineering properties make the portability claim concrete and auditable.
First, the decode that produces a per-AU pmf is re-exported through the protocols
layer for an arbitrary number of action units, taking the number of AUs and the
number of levels from array shapes rather than from hard-coded constants, while
the $0.39$ point decision is held single-source and is Feline-Grimace-Scale
specific. Second, the cache carries an explicit, versioned schema together with a
provenance record (backbone variant, pooling mode, feature dimension, DINOv2
commit, device, and torch version), so that a reported number can never hide which
backbone produced it and a different backbone can be substituted reproducibly; the
backbone loader supports a DINOv3 variant for this purpose. Neither property makes
the engine a contribution: it stays conceded plumbing.
