...
subject-exclusive cross-validation remains feasible without empty strata. Gate 1 implements early dedup (collapse\_exact\_dups + neardup) + is\_vet\_clean filter from G0 manifests/power.json (pre build\_folds/SGKF/LOIO, following standard medical pre-split hygiene); G0 power manifests are enforced first-class (orchestrator PRECONDS + folds.py strict load from manifests/power ONLY, no fallback). All preprocessing, threshold selection, temperature scaling, and abstention calibration
...
executed once. This is the \emph{CI-abort} contract of Gate~3. Reproducibility contract includes cross-env deletion-safe portable protocols surface (CI matrix ubuntu/macos + py 3.11/3.12 + make test-portable survives rm -rf engine; pre-commit-strict on G0 manifests/power) + MPS G4 correctness (blocking logit parity + CORN decode on conceded DINOv2/CORN engine before any quantitative). A DINOv3 successor seam is prepared for richer localized patch features but is not on the critical path.
...
Phase~B contribution is the capture-condition confound-attribution protocol
(\S\ref{sec:phaseA-metrics} continues into the per-AU EBPG analysis below); the per-AU VLM-vs-vet
...
number. Neither the CI-lower-bound acceptance gate (standard
clinimetrics~\cite{tractenberg2010kappa,donner2010kappa,simwright2005kappa}) nor the
...
The portable methods surface is implemented in \texttt{src/protocols/} (re-exports
of the $\kappa$ and confound protocols from \texttt{src/eval/} plus adapters for
generic reuse; see \texttt{src/protocols/standalone\_test\_corpus.py} for the
deletion-safe, zero-torch, synthetic arbitrary-AU exerciser that can survive
removal of the engine). The central gate orchestrator (\texttt{src/gates/orchestrator.py},
invoked via \texttt{make gate-orchestrate}, \texttt{gate-pipeline}, or
\texttt{gate-e2e-synthetic --synthetic}) enforces the strict run-order, writes
immutable per-run artifacts (\texttt{artifacts/gates\_run\_*.json}), aborts on
first failure, and supports synthetic toy data for CI/repro (G0 power-first,
cat-disjoint, one-directional G2, CI-LB G1-B, hash G3, blocking G4, etc.).
\texttt{Makefile} also exposes \texttt{test-portable}. These are the landed
executable embodiments of the evaluation protocol; the paper's "runnable
protocol" language is now backed by an importable, standalone, gate-enforced
surface (see also \texttt{README} quickstart and \texttt{FINAL\_DIRECTION.md}).

Concretely: \texttt{src/gates/orchestrator.py} (and synthetic mode) + e2e tests
(\texttt{tests/test\_e2e\_gates\_pipeline.py}) + CI manifests (hashed test\_manifest.sha256,
folds, power/severity) + \texttt{test-portable} (standalone + monkeypatch deletion
isolation + au\_order) + \texttt{gate-e2e-synthetic} deliver the full strict
gate-enforced contract as runnable code. This matches the evaluation chapter's
discipline and makes the confound-attribution protocol (and the guarded $\kappa$ check)
replication-survivable: the protocol surface imports and runs even if the engine
directory is deleted. A DINOv3 engine successor seam is wired into the same cache
boundary for a future A/B, off the critical path.
