...
saliency-as-confound-evidence, and a VLM judge-bias probe) that any future
corpus can re-run as a one-directional audit, reporting ``no confound detected
at this power'' rather than asserting the absence of confounding. This protocol
...
individual primitive or the underlying statistics
themselves~\cite{adebayo2022posthoc}.

The protocol and the reliability check are released as dataset-agnostic,
importable, gate-enforced executable embodiments (the ``portable methods
seam''): surfaced in \texttt{src/protocols/} (re-exports plus thin adapters for
...
(\texttt{src/protocols/standalone\_test\_corpus.py}, which validates the
confound protocol on planted positive and negative controls), and enforced by
