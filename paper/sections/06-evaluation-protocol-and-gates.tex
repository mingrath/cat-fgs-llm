...
A note on terminology used throughout. The instrument's headline contribution is the
\emph{capture-condition, power-conditioned, per-action-unit (AU) confound-attribution
protocol}; the per-AU VLM-as-rater $\kappa$ is a guarded, inspected-not-validated reliability
check, gated on its confidence-interval lower bound and held in reserve as kill-tree insurance
rather than advanced as a co-equal second pillar. The binary pain decision and the
trustworthiness wrapper are validated and cited supporting evidence respectively. The graded
...
Four structural facts bound everything in this chapter and are stated once here so that no later
section can be read as implying otherwise. (i) The corpus has not been forked: the project's
\texttt{data/} substrate is empty at the time of writing, so every corpus count below is a
\emph{projected pre-fork target} read off the unforked Roboflow source, not a measured quantity.
(ii) No independent per-AU veterinary anchor exists, which blocks the power-conditioned Gate~0
$\kappa$ feasibility check ($G0$) and the Gate~$1\text{-}B$ $\kappa$ pilot from producing any
number. (iii) The source is a Roboflow \emph{binary} (pain / no-pain) detection dataset and
carries no native $0\text{--}10$ FGS labels, so the graded layer can only ever be inspected, not
validated, against it. (iv) The $0.39$ per-class sensitivity/specificity operating point is
pre-registered as non-reportable at the budgeted sample size: the committed Gate~0 power calc
returns \texttt{reportable:false} at $n_{\text{pos}}{=}16$ (Clopper--Pearson half-width
$\approx 0.25$), so that point is carried as exploratory future work, never as a validation.
Evaluation on real cats is therefore future work; what ships now is the executable protocol
itself, exercised on synthetic and planted controls.
...
The corpus is the Roboflow \texttt{cat-pain} binary detection dataset. The counts used for
planning throughout this chapter are \emph{projected pre-fork targets} read off the unforked
source rather than measured quantities, because \texttt{data/} is empty at the time of writing
(see the four structural facts above): an anticipated $\approx 2040$ records, $\approx 1547$
distinct after collapsing $493$ exact-duplicate filenames, $336$ source clips, and pain
prevalence $\approx 12.7\%$. Because the dataset is
...
one complication is the most prolific known individual (\texttt{CAT\_01}, a projected $84$ clips,
...
subject-exclusive cross-validation remains feasible without empty strata. Following standard
medical pre-split hygiene, Gate~1 applies exact- and near-duplicate collapse and a
vet-clean filter \emph{before} the folds are built, and the Gate~0 power manifests are loaded
as a hard precondition: the fold builder reads them strictly, with no silent fallback, so the
pipeline refuses to run on an unfrozen power specification. All preprocessing, threshold
selection, temperature scaling, and abstention calibration
...
executed once. This is the \emph{CI-abort} contract of Gate~3. Two further reproducibility
clauses are blocking and are detailed in \S\ref{sec:repro}: the portable protocol surface is
verified to run cross-platform and to survive deletion of the engine directory, and the Gate~4
compute-correctness checks (logit parity and CORN decode on the conceded engine) must pass
before any quantitative result is believed.
...
This fixed-recall choice is a deliberate, cited design choice for the wrapper rather than a
headline contribution (the headline is the confound-attribution protocol): a
...
Phase~B evaluates the AU-level engine output and the trustworthiness wrapper. The headline
Phase~B contribution is the capture-condition confound-attribution protocol: a two-leg,
power-conditioned, one-directional audit combining an image-side FGS-BG-Gap counterfactual
(built on background-swap robustness primitives~\cite{xiao2021noise,moayeri2022comprehensive})
with a per-AU energy-based pointing-game (EBPG) saliency leg (built on the Score-CAM
faithfulness primitive~\cite{wang2020scorecam}). Its power-conditioned, equivalence-style audit
hygiene is credited to prior work~\autocite{huanghooker2026fairness,singh2023samplesize}, and
the one-directional ``no confound \emph{detectable at this power}'' stance is supported by the
finding that post-hoc attribution can fail to surface unknown spurious signal~\cite{adebayo2022posthoc};
novelty is claimed only for the \emph{assembly} and the per-AU EBPG-as-confound-evidence
instantiation. A separate segment-based per-AU saliency-alignment framework for animal affective
computing~\cite{lencioni2025segment} is the closest prior per-AU saliency work; our leg differs
in repurposing per-AU EBPG specifically as one-directional confound evidence rather than as an
explanation-quality measure. The per-AU VLM-vs-vet
agreement $\kappa$ is a guarded reliability check whose result is pending --- it yields no number
until an independent per-AU vet anchor exists ($G0$), so until then it ships as a runnable,
CI-lower-bound-gated protocol rather than a delivered measurement, and it is held as kill-tree
insurance rather than presented as a co-equal pillar. Everything concerning the graded
$0\text{--}10$ sum is computed and reported as inspection-only.

\subsection{Per-AU VLM-as-rater quadratic kappa (guarded reliability check)}

This guarded check asks whether a frozen vision-language model can weak-label the five
...
number. The CI-lower-bound machinery is nonetheless exercised today on synthetic and planted
controls --- the bootstrap CI-lower-bound estimator (\texttt{bootstrap\_qwk\_lb} in
\texttt{src/eval/kappa.py}) is validated against constructed agreement patterns --- so the gate
is executable even though no real $\kappa$ number can exist until the vet anchor does. Neither the
CI-lower-bound acceptance gate (standard
clinimetrics~\cite{tractenberg2010kappa,donner2010kappa,rotondi2012kappa,rotondi2018kappasize,simwright2005kappa})
nor the
rubric-paraphrase independence guard (published as a reliability test for LLM
judges~\cite{weng2026policy}) is claimed as a novel increment; both are instantiated here as
audit hygiene. For each AU the protocol computes the quadratic-weighted
...
fold-to-fold variance. Crucially, this $0.39$ sensitivity/specificity point is \emph{not
reportable at the budgeted sample size}: the committed Gate~0 power calc
(\S\ref{sec:gates}, $G0$) returns \texttt{reportable:false} at $n_{\text{pos}}{=}16$, with a
Clopper--Pearson half-width of $0.2535$. The point is therefore carried as exploratory only,
conditional on a future enlarged vet anchor, and is never advanced as a validation.
Evangelista's published operating point is cited solely to \emph{source}
the recall target; it is prior-art context, never a head-to-head comparison.

\subsection{Welfare-asymmetric decision curve (cited wrapper plumbing)}

The wrapper's primary reported artifact --- cited supporting plumbing, not a headline
contribution --- is a decision-curve / net-benefit analysis~\cite{vickers2006dca}
...
\]
makes the welfare asymmetry explicit; this welfare-asymmetric framing is the cited design choice
that distinguishes the wrapper from generic calibration hygiene, and is not advanced as a
...
procedure~\cite{angelopoulos2021ltt} fit inside the training folds. It is reported strictly as a
\emph{one-sided $95\%$ lower bound} at each abstention rate; the word ``guaranteed'' is banned
from every sentence describing it. The Gate~0 NPV sizing ($G0$) should be read as
\emph{placeholder-illustrative}, not as a certified result: it is computed over a placeholder vet
budget ($\texttt{vet\_budget}{=}120$, the unconfirmed default echoed from \texttt{configs/power.yaml})
against zero forked data ($\texttt{total\_faces}{=}84$, with $\texttt{npv\_target}{=}0.90$). The
band is therefore \emph{not yet certified}---not because the arithmetic forbids a useful floor,
but because the budget is a placeholder over zero measured data. The $0.95$ arithmetic is retained
only to justify two reporting rules: the ban on ``guaranteed,'' and the use of a one-sided $95\%$
lower bound as the only honest summary the eventual (non-placeholder) budget could defend, and at
...
Phase~B guarded reliability check (gated) \\
\addlinespace
RPS on $0\text{--}10$ sum &
...
Phase~B wrapper (cited plumbing) \\
\addlinespace
...
\midrule
$G0$ & Committed single-integer vet budget (placeholder $120$, $\geq 50$ positive) + $\kappa$
floors + $\rho$-band decision; three zero-data power calcs. \emph{Committed outcome:} $\kappa$ CI
half-width $\leq 0.15$ met; NPV band sized as placeholder-illustrative over a placeholder budget
(reported as one-sided $95\%$ LB, never ``certified''); $0.39$ CI \texttt{reportable:false} at
$n_{\text{pos}}{=}16$ (CP half-width $0.25$) &
CPU ($\tfrac{1}{2}$ day) &
$0.39$ point ships as exploratory future work, not a result; NPV abstention is
placeholder-illustrative (no ``guaranteed''); not a kill \\
\addlinespace
...
CPU, no vet &
Portable protocol $\Rightarrow$ proceed (this is the headline leg); non-portable $\Rightarrow$
threat-to-validity \\
\addlinespace
...
binary pain decision $+$ the headline confound-attribution protocol $+$ the guarded
$\kappa$ reliability check (result pending the independent vet anchor, retained as kill-tree
insurance) $+$ the welfare decision curve $+$ the lower-bound abstention curve, with graded-CORN
shipped as an inspected-not-validated artifact. Every gate either advances this product or pivots
cleanly onto it; if the confound leg degrades, the $\kappa$ check becomes the sole-survivor
headline.
...
\begin{enumerate}
  ...
  \item The DINOv2$+$CORN engine is stated to be \textbf{conceded plumbing, not a novel
        contribution}; the headline contribution is the power-conditioned, per-AU
        confound-attribution protocol, supported by a guarded VLM-as-AU-rater $\kappa$
        reliability check and a cited welfare-asymmetric wrapper. Novelty is claimed for the
        \emph{assembly} and the per-AU EBPG-as-confound-evidence instantiation, never for any
        individual primitive or for the underlying audit statistics.
\end{enumerate}
...
The confound-attribution protocol and the guarded $\kappa$ check are released as an importable,
gate-enforced module that is validated today on synthetic and planted positive/negative controls
and survives deletion of the engine directory, so the chapter's ``runnable protocol'' language is
backed by code that runs independently of the conceded backbone. A strict gate orchestrator
enforces the run-order described above and writes immutable per-run artifacts, aborting on first
failure.
