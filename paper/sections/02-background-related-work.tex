...
DINOv2 ViT-S/14 encoder purely as a fixed feature extractor---the register
variant (\texttt{dinov2\_vits14\_reg}) as the field default, because registers
suppress attention artifacts that hurt the dense, localized features per-AU FGS
scoring depends on (orbital, ear, muzzle sub-regions), A/B'd against the plain
variant before locking ViT-S/14: the backbone is
...
kappa-acceptance discipline established for clinical ratings
\cite{tractenberg2010kappa,simwright2005kappa} and its sample-size planning
counterpart \cite{donner2010kappa,rotondi2012kappa,rotondi2018kappasize}; the
same ordinal-AU kappa/ICC-with-CI practice anchors the feline-pain domain
...
\cite{xiao2021background,xiao2021noise} demonstrates that models routinely
...
not ours. The
\emph{protocol}---FGS-BG-Gap plus per-AU EBPG plus VLM judge-bias probe, runnable
on any future facial-pain corpus and validated today on planted positive and
negative controls---is the headline contribution of this work, distinct from any
...
capture-condition confound-attribution protocol (FGS-BG-Gap plus per-AU
EBPG-as-confound-evidence plus a VLM judge-bias probe), one-directional by
construction and validated today on planted controls. Its claimed novelty is the
...
one-directional / power-conditioned audit statistics themselves
\cite{adebayo2022posthoc,huanghooker2026fairness,singh2023samplesize}.
...
\cite{tractenberg2010kappa,simwright2005kappa,donner2010kappa}, with a
published rubric-paraphrase guard \cite{weng2026policy}, neither claimed as a
novel increment. The DINOv2-plus-CORN engine carrying these methods is conceded as
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{3.0cm}p{3.1cm}p{3.3cm}cc@{}}
    ...
    \textbf{This work (headline)}
      ...
      & \emph{FGS-BG-Gap + per-AU EBPG + VLM judge-bias (one-directional)}
      & Audit & \textbf{Yes} \\
