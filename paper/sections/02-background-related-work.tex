...
measurement, not the act of prompting a VLM, is the contribution; the rater is selected by
...
$\{0,1,2\}$ without weights is a bug to avoid). The headline method of this
project is exactly such a per-AU quadratic weighted kappa between the VLM and
the veterinary anchor---shipped as a runnable protocol, with the number itself
...
no prior feline-pain study produced: (i) a protocol for measuring whether a
frozen VLM can weak-label FGS action units at human-rater agreement, expressed as five
...
metrics} critique: the two surviving claims are a reusable VLM-vs-vet
kappa protocol (whose result is pending the independent per-AU vet anchor) and a
reusable audit protocol, both portable to
