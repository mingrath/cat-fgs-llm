...
The thesis pursues four numbered objectives, ordered from the two headline
\emph{portable methods} down to the conceded engineering substrate.

\begin{enumerate}
  \item \textbf{Measure VLM--vet agreement on FGS action units.} Determine whether
    a frozen vision--language model (VLM) can weak-label the five FGS action units
    at \emph{human-rater agreement}, quantified as a per-AU quadratic-weighted
    Cohen's $\kappa$ against a veterinary anchor, reported with a confidence-interval
    lower bound. This is a forward-looking method finding---\emph{can} a frozen
    VLM weak-label feline FGS AUs at rater agreement---not an audit of one
    corpus's particular labels.
  \item \textbf{Build a portable confound-attribution protocol.} Construct a
    reusable, one-directional capture-condition audit (the FGS--BG--Gap and
    per-AU energy-based pointing-game, EBPG) that any future facial-pain-scorer
    corpus can run to detect---never to ``rule out''---entanglement between the
    pain label and acquisition context.
...
Objectives~1 and~2 are the two transportable contributions of the thesis;
Objective~3 is the honest decision floor that stands on its own even if the graded
layer is dropped; Objective~4 is built for completeness and future extension but
is firewalled out of every validated results table. The contribution map is
summarised in Figure~\ref{fig:contribution-map}.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    % two portable methods (the headline)
    \node[head] (m1) {\textbf{Method 1}\\VLM-as-AU-rater\\per-AU quadratic $\kappa$\\vs.\ vet (CI-lower-bound-gated)};
    \node[head, right=1.0cm of m1] (m2) {\textbf{Method 2}\\Capture-condition\\confound-attribution\\(FGS-BG-Gap + per-AU EBPG)};
    \node[lbl, above=0.15cm of m1.north east] {portable, dataset-agnostic --- the claimed contributions};
    ...
    \node[lbl, below=0.1cm of op.south east] {trustworthiness wrapper --- the frame};
    ...
    \node[engine, below=1.5cm of op.south east, anchor=north, text width=8.4cm] (eng)
      {\textbf{Conceded engine (plumbing --- never claimed novel):}
       frozen DINOv2 ViT-S/14 $\rightarrow$ 5 per-AU CORN heads $\rightarrow$
  ...
  \end{tikzpicture}
  \caption[Thesis contribution map]{Contribution map. The two headline
    deliverables are \emph{portable methods} (top): a VLM-as-AU-rater per-AU
    $\kappa$ protocol and a capture-condition confound-attribution protocol. The
    trustworthiness wrapper (middle) supplies the welfare-asymmetric operating
    point and the abstention floor. The DINOv2$+$CORN engine (bottom) is conceded
    plumbing and is never claimed as a contribution; its graded output ships
    inspected-not-validated. (Placeholder schematic.)}
...
binary-plus-abstention floor with the confound protocol; the two portable-method
headlines still stand, because a mediocre $\kappa$ is itself a real first
measurement.
...
firing favourably. Its expected contributions are three released, dataset-agnostic
artefacts, each of which stands as a portable method rather than a corpus-bound
number:

\begin{enumerate}
  \item \textbf{A VLM-as-AU-rater $\kappa$ protocol and result.} Released code that
    ...
    bounds. The forked corpus's numbers merely \emph{instantiate} the protocol;
    the deliverable is the reusable measurement and the one fact-about-the-world
    it produces---whether a frozen VLM weak-labels feline FGS action units at
    human-rater agreement.
  \item \textbf{A capture-condition confound-attribution protocol.} The
    one-directional FGS--BG--Gap plus per-AU EBPG audit, runnable on any future
    facial-pain-scorer corpus, whose deliverable is the protocol itself rather
    than a verdict about one dataset.
  \item \textbf{A welfare-asymmetric decision-curve and abstention floor.} An
    ...
    contribution that distinguishes the trustworthiness wrapper from generic
