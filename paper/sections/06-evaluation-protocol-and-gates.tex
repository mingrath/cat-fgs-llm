...
Phase~B number is a \emph{gated, validated} quantity --- the per-AU VLM-vs-vet agreement
$\kappa$; everything concerning the graded $0\text{--}10$ sum is computed and reported as
inspection-only.
...
FGS action units at human-rater agreement. For each AU we compute the quadratic-weighted
agreement \cite{cohen1968kappa} between the model's $\{0,1,2\}$ rating and a veterinary
anchor, \emph{bootstrap its confidence interval, and gate and report on the CI lower bound}
...
\emph{not} validation against the vet.
...
\texttt{artifacts/gateN/PASS}. ``Pivot'' rows can redirect the spine to the binary fallback
without killing the project; ``Blocking'' rows must turn green before any downstream number is
believed.}
...
\emph{fragile}, which is reported as such, and the build defaults to the binary fallback rather
...
binary pain decision $+$ the $\kappa$-as-method finding $+$ the confound-attribution protocol
