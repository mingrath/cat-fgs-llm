...
\begin{enumerate}
  ...
  (\S\ref{sec:vlm-au-rater-kappa}).} A \emph{protocol} for measuring whether a frozen VLM can
  weak-label feline FGS action units at human-rater agreement (result pending the independent vet anchor of Gate~0), expressed as five
...
project ships the binary-plus-wrapper spine --- the likely v1 ship that stands today --- and the kappa protocol ships as
a method regardless. All of this section is hosted-API-only --- no local
...
The first portable contribution is a \emph{protocol}: can a frozen VLM
weak-label feline FGS action units at human-rater agreement? The deliverable is
the \emph{protocol; the result is pending the independent vet anchor that does not yet exist} --- released code that scores any face corpus's
per-AU VLM labels against a veterinary anchor and reports five quadratic kappas
with confidence-interval lower bounds. No numbers can be obtained on the present
corpus, which is binary \texttt{pain}/\texttt{no\_pain} and cannot yield $0/1/2$ per-AU ground truth; the result is pending the independent vet anchor (Gate~0). The protocol is
immune to the
...
reading from being mistaken for validation of the $0.39$ flag.
...
calibrated binary spine plus abstention plus the confound protocol, and the
per-AU kappa protocol remains a publishable method --- one that yields a real
method finding once the independent anchor exists, even at a mediocre kappa.
