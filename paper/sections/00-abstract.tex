...
estimate --- a protocol whose result is pending an independent per-AU
veterinary anchor that does not yet exist (the current binary pain/no-pain
labels cannot yield $0/1/2$ AU ground truth), so it ships today as a
runnable protocol rather than a reported number; and \textbf{(ii)} a \emph{capture-condition confound-attribution
