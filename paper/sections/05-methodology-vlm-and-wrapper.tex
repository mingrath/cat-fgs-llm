\chapter{Methodology II: VLM Supervision, the Confound-Attribution Headline, and the Trustworthiness Wrapper}
\label{ch:vlm-wrapper}

The preceding chapter specified the \emph{engine}: a frozen DINOv2 ViT-S/14
backbone (with \texttt{dinov2\_vits14\_reg}, the register variant, as the field default since registers suppress attention artifacts that hurt the dense, localized per-AU features; the plain variant is A/B'd before locking ViT-S/14) feeding five per--action-unit (AU) CORN ordinal heads, decoded to
...
The deliverables below are released as dataset-agnostic, runnable code that any
future facial-pain-scoring corpus can re-run, and they are arranged
\emph{asymmetrically} around a single headline:

\begin{enumerate}
  \item \textbf{Headline --- the capture-condition confound-attribution
  protocol (\S\ref{sec:confound-protocol}).} A power-conditioned,
  one-directional, \emph{per-AU} audit over two components --- the
  \emph{FGS-BG-Gap} counterfactual plus the per-AU energy-based pointing game
  (EBPG) as saliency-as-confound-evidence --- that detects whether the scorer is
  reading acquisition context rather than grimace. (A third, VLM judge-bias axis
  is fully coded but exercised only on synthetic controls; its application to a
  real VLM is named as future work, \S\ref{sec:confound-protocol}, and is not
  part of the headline.) This is the strong leg: it is
  exercised today on planted positive and negative controls, and the novelty is
  the \emph{assembly} plus the per-AU EBPG-as-confound-evidence step plus the
  instantiation of equivalence-style audit hygiene --- never any individual
  primitive and never the one-directional or power-conditioned statistics
  themselves.
  \item \textbf{Supporting reliability check --- VLM-as-AU-rater $\kappa$
  (\S\ref{sec:vlm-au-rater-kappa}).} A guarded, inspected-not-validated check of
  whether a frozen VLM can weak-label feline FGS action units at human-rater
  agreement (result pending an independent vet anchor), expressed as five per-AU
  quadratic-weighted kappas against a veterinary anchor, gated on the
  \emph{confidence-interval lower bound}. The CI-lower-bound gate is textbook
  clinimetrics and the rubric-paraphrase guard is published; neither is claimed
  as a novel increment. The check is retained as kill-tree insurance --- it
  becomes the sole-survivor headline only if the confound leg degrades --- and
  is ranked below the confound protocol, not co-equal with it.
\end{enumerate}

Supporting these is the \emph{cited trustworthiness wrapper} --- a
welfare-asymmetric decision curve, a fixed-high-sensitivity operating point, and
a defer-to-vet abstention boundary reported as a one-sided lower bound ---
assembled from cited primitives as plumbing and supporting evidence, not as a
...
run is \emph{blocked} until the kappa reliability check (Gate~1-B,
\S\ref{sec:vlm-au-rater-kappa}) returns \textsc{go}; on \textsc{no-go} the
project ships the binary-plus-wrapper spine --- the likely v1 ship that stands today --- with the confound-attribution headline intact, and the kappa check ships as
a guarded reliability result regardless. All of this section is hosted-API-only --- no local
...
\section{Supporting reliability check: VLM-as-AU-rater kappa}
\label{sec:vlm-au-rater-kappa}

The supporting reliability check asks a single question: can a frozen VLM
weak-label feline FGS action units at human-rater agreement? The deliverable is
a guarded, inspected-not-validated check --- the \emph{result is pending the
independent vet anchor that does not yet exist} --- released code that scores any
face corpus's per-AU VLM labels against a veterinary anchor and reports five
quadratic kappas with confidence-interval lower bounds. No numbers can be
obtained on the present corpus, which is binary \texttt{pain}/\texttt{no\_pain}
and cannot yield $0/1/2$ per-AU ground truth; the result is pending the
independent vet anchor. The check sits \emph{below} the confound-attribution
headline of \S\ref{sec:confound-protocol}; it is retained as kill-tree insurance
that would become the sole-survivor headline only if the confound leg degrades.
Its machinery follows standard, published clinimetrics rather than introducing a
novel statistic: the CI-lower-bound acceptance gate is textbook
\cite{tractenberg2010kappa,donner2010kappa,rotondi2012kappa,rotondi2018kappasize,simwright2005kappa},
the CI half-width planning uses \texttt{kappaSize}
\cite{rotondi2018kappasize}, and the rubric-paraphrase independence guard is
published \cite{weng2026policy}. What the check contributes is the
\emph{instantiation} of this hygiene on the VLM-as-AU-rater question in the
feline-FGS domain, where no prior feline-pain work has reported it
\cite{evangelista2021fgs,cheng2020fgs}.

\paragraph{Interpretation guard.} A high per-AU kappa measures weak-labeling
\emph{capability} only if \emph{both} (i) the vet anchor is independent of the
VLM and (ii) the rubric handed to the VLM differs from the rubric the vet scored
from; otherwise it measures \emph{rubric-following}, not weak-labeling skill.
The second condition is live here, because the VLM is prompted with the verbatim
Evangelista descriptors (\S\ref{sec:vlm-rubric}): if the anchoring veterinarian
also scores from those same descriptors, a high kappa is consistent with shared
rubric-following alone. Every reported kappa run must therefore state which of
the two it measures, and the firewall (\S\ref{sec:firewall}) keeps either
reading from being mistaken for validation of the $0.39$ flag. This
rubric-paraphrase independence guard is a published reliability test, not a novel
increment of this work \cite{weng2026policy}; here it is instantiated on the
VLM-as-AU-rater question.
...
heavily as a $0$-versus-$1$ disagreement --- and is the standard ordinal
agreement choice, not a novel statistic. A one-sided
...
by a power calculation. Gating acceptance on a kappa CI lower bound is textbook
clinimetric practice rather than a contribution of this work
\cite{tractenberg2010kappa,donner2010kappa,rotondi2012kappa,rotondi2018kappasize,simwright2005kappa},
and the canonical $0.60$ acceptable lower bound is adopted from that literature
\cite{simwright2005kappa}. Table~\ref{tab:kappa-gate} states the pre-registered
decision rule. The floors shown are the planning defaults that the Gate~0 power
calculation (via \texttt{kappaSize} \cite{rotondi2018kappasize}) may overwrite;
they are never hard-coded in the gate logic.
...
\paragraph{The pilot justifies the labeler once the anchor exists.} The choice of VLM is settled
...
the root of the supervision pipeline. A \textsc{no-go} on orbital/ear/head is a
\emph{pivot}, not a kill: the project drops the $0$--$10$ layer and ships the
calibrated binary spine plus abstention plus the headline confound protocol, and the
per-AU kappa check remains a guarded, inspected-not-validated reliability result
--- one that reports a real reliability finding once the independent anchor
exists, even at a mediocre kappa.
...
\begin{itemize}
  ...
  vet-confirmed labels.} These are the spine numbers, reported with
  Clopper--Pearson or bootstrap intervals and the distinct-pain-cat denominator.
  They are \emph{not} a producible validated result at the present budget: the
  committed Gate~0 power calculation flags the $0.39$ per-class sens/spec point as
  \texttt{reportable:false} at the budgeted anchor ($n_{\text{pos}}=16$,
  Clopper--Pearson half-width $0.2535$), so this leg ships as exploratory /
  future work pending an enlarged independent vet anchor (see the Gate-0 and
...
\node[valid, right=28mm of vet] (sens) {sens/spec at $0.39$\\(validation-path quantity;\\\texttt{reportable:false} at budgeted $n$)\\Clopper--Pearson CI};
...
\end{tikzpicture}
\caption[Circularity firewall]{The circularity firewall. Sensitivity
and specificity at the $0.39$ cut are the validation-path quantity, estimated
\emph{only} on vet-confirmed labels (solid path), but they are pre-registered
\texttt{reportable:false} at the budgeted anchor ($n_{\text{pos}}=16$,
Clopper--Pearson half-width $0.2535$) --- exploratory pending an enlarged vet
anchor. The quadratic-weighted kappa against VLM labels is a
...
\section{Cited wrapper: the welfare-asymmetric decision curve}
\label{sec:decision-curve}

The wrapper's primary reported artifact --- cited supporting plumbing, not a
headline contribution --- is \emph{not} a calibration plot but a
welfare-asymmetric decision curve, assembled from cited decision-theory
primitives as supporting evidence rather than as a headline contribution. Pain
assessment is a cost-asymmetric
...
\emph{treat-none} across the welfare-relevant low-$p_t$ band; this applies
standard decision-curve analysis to the cat-pain setting as supporting clinical
evidence, and it is the floor that survives every downstream null. If the
abstention power budget fails (Gate~0), this curve becomes the primary clinical
deliverable of the wrapper.
...
sweeps the undertreat\,:\,overtreat harm ratio as a range. The claim the curve
supports is that the model curve dominates both treat-all and treat-none across the
...
\section{Cited wrapper: the fixed pain-recall operating point}
...
confidence intervals on out-of-fold predictions --- though, as at the firewall
(\S\ref{sec:firewall}), at the budgeted anchor the $0.39$ per-class sens/spec CI
is pre-registered \texttt{reportable:false} ($n_{\text{pos}}=16$, half-width
$0.2535$), so this operating-point estimate is exploratory pending an enlarged
vet anchor. Fixing recall first is the
...
\section{Cited wrapper: defer-to-vet abstention as a one-sided NPV lower bound}
...
appears. The companion Gate~0 NPV sizing
(\texttt{calc\_b}, $\text{npv\_target}=0.90$) is reported here strictly as a
\emph{placeholder-illustrative} computation: it is run over a \emph{placeholder}
vet budget of $120$ faces ($\geq 50$ pain-positive) and a total of $84$ faces
against an empty corpus that has not yet been forked. Accordingly, any band the
sizing appears to clear is \emph{not yet certified --- because the budget is a
placeholder over an empty corpus, not because the arithmetic forbids a useful
floor}. A certified band requires re-running the sizing on a real,
non-placeholder budget over measured data; until then the abstention curve ships
as illustrative protocol, with the welfare-asymmetric decision curve of
\S\ref{sec:decision-curve} carrying the wrapper's primary reported artifact ---
cited supporting plumbing, not a headline contribution. This
is a placeholder status, not a kill.
...
``Guaranteed'' is never used; the companion Gate~0 NPV sizing is
placeholder-illustrative (run over a placeholder vet budget of $120$ faces, total
$84$, against an empty corpus), so any cleared band is not yet certified --- a
placeholder over zero data, not an arithmetic impossibility. The actual curve is
to be produced from vet-confirmed held-out scores.}
\label{fig:abstention}
\end{figure}

\section{Headline: the confound-attribution protocol}
\label{sec:confound-protocol}

The headline contribution is a power-aware, per-AU capture-condition
confound-attribution protocol: a reusable, one-directional audit that any future
facial-pain-scorer corpus can run to ask whether the scorer is reading
acquisition context --- the cage, the clinic, the lighting --- rather than the
grimace itself. Single-source data makes this audit essential: pain and capture
condition are plausibly entangled, and a scorer that exploits that entanglement
would post strong accuracy while learning nothing about pain. The deliverable is
the \emph{protocol}, released as code and exercised today on planted positive
and negative controls, not a verdict about this particular corpus. The novelty
is the \emph{assembly} of these components, the per-AU
EBPG-as-confound-evidence step, and the instantiation of equivalence-style audit
hygiene
\cite{huanghooker2026fairness,singh2023samplesize}; no individual primitive is
claimed as novel, and the one-directional, power-conditioned, and
equivalence-style statistics themselves are adopted from that literature rather
than introduced here. The protocol is distinct from the positively-framed
per-AU saliency leg of prior animal-affect explainability work
\cite{lencioni2025segment} and from background-context-bias audits in the human
affect-VLM domain \cite{context2025becki}.
...
caveat appended after the fact, and it is consistent with evidence that post-hoc
explanations can fail to surface unknown spurious correlations
\cite{adebayo2022posthoc}; the contribution is the disciplined,
power-conditioned framing of that limit, not the statistical machinery behind
it.
...
background-robustness counterfactual primitive~\cite{xiao2021noise} (and the
foreground/background sensitivity analyses of \citealp{moayeri2022comprehensive})
to the FGS setting; the bg-swap primitive itself is prior work and is not claimed
as novel.
...
head fire on the ear?'' --- a per-AU localization-faithfulness measurement
re-purposed here \emph{as confound evidence}, which is the increment that
distinguishes it from the positively-framed per-AU saliency leg of prior
animal-affect explainability work \cite{lencioni2025segment}. Where landmark
coverage is partial, EBPG is reported only on the covered subset with the
denominator stated.

\paragraph{Future work: a VLM judge-bias axis.} A third axis --- a paraphrase /
context-perturbation probe of whether the VLM weak-labeler's scores track the
acquisition context rather than the grimace --- is fully coded but exercised
\emph{only on synthetic controls}; applying it to a real hosted VLM is named as
future work and is deliberately \emph{not} part of the two-component headline.
It is retained here because the same one-directional, power-conditioned framing
extends to the labeler as well as the engine; the human-affect-VLM background of
this kind of context-bias audit is noted in \cite{context2025becki}.
...
confound-attribution protocol (the chapter's headline contribution), portable
and one-directional. Branch (a) FGS-BG-Gap measures the score shift under a swapped
...
\S\ref{sec:decision-curve}, which is the wrapper's primary reported artifact ---
cited supporting plumbing, not a headline contribution. This
keeps the contribution clearly located in the headline confound-attribution
protocol, supported by the guarded kappa reliability check and the cited
wrapper, with generic calibration relegated to hygiene.
...
flag computed in code so the labeler never makes a clinical decision. The
headline is a power-conditioned, one-directional, per-AU confound-attribution
protocol; the per-AU VLM-versus-vet quadratic-weighted kappa, gated on the CI
lower bound, is a supporting, guarded, inspected-not-validated reliability check
ranked below it and retained as kill-tree insurance. A welfare-asymmetric
decision curve, a fixed pain-recall operating point, and a one-sided
NPV-lower-bound abstention boundary are the cited wrapper that frames the
clinical use, while the circularity firewall ensures that validation rides on
vet-confirmed labels only and that agreement-against-VLM is never mistaken for
validation. The engine remains conceded plumbing; the contribution lives here,
and every claim in this chapter is stated as a protocol and a decision rule, not
as a result. The kappa CI-lower-bound gate and the rubric-paraphrase guard are
standard, published clinimetrics and reliability tests, not novel increments;
the novelty is the assembly, the per-AU EBPG-as-confound-evidence step, and the
instantiation of equivalence-style audit hygiene.
