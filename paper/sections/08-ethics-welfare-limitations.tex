...
knee, and the wrapper's primary reported artifact (cited supporting plumbing, not
a headline contribution) is a harm-ratio-weighted decision curve
...
VLM-as-AU-rater kappa reliability check, with the welfare-asymmetric decision
curve and NPV-lower-bound abstention released alongside as cited supporting
plumbing rather than a headline contribution --- all of which ship as runnable code
...
this $n$ the severity tail is essentially unestimable, and a data-fitted cross-AU
error-correlation matrix would launder noise as rigor. The NPV is reported
strictly as a one-sided $95\%$ lower bound; the word ``guaranteed'' is banned,
because a finite-sample lower bound is never a certainty at any $n$. And any band
the placeholder sizing appears to clear is \emph{not yet certified}, because the
vet budget ($\mathit{vet\_budget}{=}120$) is an unconfirmed placeholder over zero
data, not because the arithmetic forbids a useful floor. We respond by budgeting
the anchor up front and by demoting any claim the budget cannot support
\emph{on paper now} rather than discovering its weakness later.
...
\emph{synthetic data with planted positive and negative controls}, split across
two harnesses: a standalone harness
(\texttt{src/protocols/standalone\_test\_corpus.py}) drives the
no-confound-at-this-power branch, asserting that a deliberately clean control
reads no confound detected at this power, and a planted-positive recovery test
(\texttt{tests/test\_confound\_recovery.py}) asserts that a known
above-threshold capture-condition shift is flagged detected. Together they show
the confound-attribution protocol both stays silent on a clean control and fires
on a deliberately planted confound, not yet on real feline imagery. The
data directory (\texttt{data/}) ships empty, no per-AU veterinary anchor exists, the
corpus is Roboflow binary-only, and the $0.39$ operating point is currently
flagged \texttt{reportable:false} by the Gate-0 power calculation at
$n_{\text{pos}}{=}16$ (Clopper--Pearson half-width $\approx 0.25$). We therefore present this work as a
...
portable protocol itself rather than any verdict about this specific corpus. The
novelty we do claim is narrow: it is the \emph{assembly} of these published
primitives into a power-conditioned audit, together with the use of per-AU EBPG
saliency as confound evidence; we claim novelty in neither any individual primitive
nor in the equivalence and power statistics themselves, which are credited to the
fairness-audit literature~\cite{huanghooker2026fairness,singh2023samplesize}. The
per-AU saliency leg adapts a region-attributed saliency reading whose feline
landmark-segmentation lineage is distinct from ours~\cite{lencioni2025segment}: we
read it as confound evidence rather than as a localization product.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.27\linewidth} p{0.30\linewidth} p{0.33\linewidth}@{}}
    ...
    Empty \texttt{data/}, no vet anchor, Roboflow binary-only; $0.39$ point
    flagged \texttt{reportable:false} by the Gate-0 power calc at
    $n_{\text{pos}}{=}16$ (Clopper--Pearson half-width $\approx 0.25$) &
    Ship as a methods paper: clean control reads no confound
    (\texttt{standalone\_test\_corpus.py}), planted-positive shift flagged
    detected (\texttt{test\_confound\_recovery.py}); real-cat application named as
    future work \\
    \addlinespace
