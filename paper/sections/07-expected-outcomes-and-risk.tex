...
No experiments have been run. This chapter contains no measured results, no
accuracies, no $\kappa$ values, no calibration numbers, and no figures of our own
curves. It states, per gate, the \emph{decision rule} and the gate-conditioned
outcome (\textsc{go} / \textsc{pivot} / \textsc{collapse}), the modal product we
plan to ship, and the kill criteria under which we pivot. Every numeric value that
appears (e.g. \evangelistanums) is drawn from the prior literature and is
explicitly attributed to its authors as a \emph{background anchor}, never as a
result of this work.
\end{quote}
...
trustworthiness wrapper}, accompanied by two portable methods, shipped on an
explicitly-conceded engine. The graded $0$--$10$ Feline Grimace Scale (FGS) layer
...
Table~\ref{tab:modal-components}. Five are validated claims; the sixth (the graded
CORN layer) is never entered into a validated results table.
...
2 & $\kappa$-as-method &
  ...
  its \emph{CI lower bound}; the protocol is the deliverable & Yes (headline~1) \\
3 & Confound-attribution protocol &
  FGS-BG-Gap $+$ per-AU EBPG \citep{wang2020scorecam,xiao2021background},
  \emph{one-directional} (``no confound detected at this power'') & Yes (headline~2) \\
...
5 & LB-abstention curve &
  ...
  \citep{angelopoulos2021ltt}; ``guaranteed'' is banned & Yes (exploratory if G0 unmet) \\
...
The two \emph{portable methods} --- Component~2 (VLM-as-AU-rater $\kappa$) and
Component~3 (the confound-attribution protocol) --- are the only claimed
contributions, and both ship as released, runnable, dataset-agnostic code. The
calibration, RPS, ClasswiseECE, and conformal machinery are \emph{supporting
evidence}, never headlines. The engine (frozen DINOv2 ViT-S/14 --- with the \texttt{\_reg}
...
\node[ship, below=of g6] (modal) {Modal product:\\binary $+$ wrapper $+$ $\kappa$-method\\$+$ confound protocol\\$+$ graded (inspected-only)};

\foreach \a/\b in {g0/g1, g1/g2, g2/g3, g3/g4, g4/g5, g5/g1b, g1b/g6, g6/modal}
  \draw[flow] (\a) -- (\b);

% branch outcomes on the right
\node[branch, right=18mm of g0] (g0b) {G0(b) fail:\\NPV band ships\\\textsc{exploratory}};
...
but still ships the calibrated binary spine, the $\kappa$-as-method headline, and
the confound protocol (dashed arrow). G4/G5 are blocking correctness gates --- a
...
\midrule
G0(b) &
  Budget cannot certify a useful NPV band &
  \textsc{proceed}; the abstention curve (Component~5) is demoted to
  \emph{exploratory}; ``guaranteed'' never appears. Not a kill --- relocates the
  deliverable onto the welfare decision curve. \\[2pt]
G2 &
  Audit damning but yields only ``CAT\_01 is confounded'' (non-portable) &
  Demote the audit to a threat-to-validity paragraph; the $\kappa$ method then
  carries the headline alone. If G1-B \emph{also} fails, the honest paper is a
  confound/audit note (venue pre-registered now). \\[2pt]
G2 &
  Confounding detected as a \emph{transportable protocol} &
  \textsc{proceed} --- a publishable co-headline even if everything downstream is
  null \citep{xiao2021background}. \\[2pt]
G1-B &
  ...
  binary $+$ abstention $+$ confound audit. $\kappa$-as-method and the wrapper
  still stand (a real first per-AU $\kappa$ protocol is publishable, with its result
pending the independent vet anchor). \\[2pt]
G1-B &
...
$\rho$-band &
  ...
  Declare the gate \emph{fragile}; default to the binary spine rather than
...
\emph{no branch is a project-killing null}. The two portable methods are designed
to be independently publishable: a runnable per-AU $\kappa$ protocol is still the
first protocol for measuring whether a frozen vision-language model can weak-label
feline FGS action units at human-rater agreement (the result is pending an
independent per-AU vet anchor, and is only a weak-labeling-capability measurement
if that anchor is independent and the VLM's rubric differs from the vet's ---
otherwise it measures rubric-following), and a confounding signal detected as a
transportable protocol is a publishable co-headline regardless of downstream
outcomes. The binary-plus-wrapper spine is the floor that stands today --- the
likely v1 ship --- with the graded layer as upside conditional on the Gate-1-B
$\kappa$ pilot passing.
...
modest vet-confirmed anchor (a pre-Gate-0 placeholder of ${\sim}120$--$150$
confirmed images, ${\geq}50$ pain-positive; the exact integer is fixed by the
Gate~0 power calc and committed before any vet hour is spent). Gate~0 runs three
zero-data power calculations: (a) the number of faces needed for a per-AU $\kappa$
CI half-width $\leq 0.15$; (b) the number needed for a Learn-Then-Test /
conformal-certified NPV $\geq 0.90$ at an abstention rate $\leq 40\%$
\citep{angelopoulos2021ltt}; and (c) whether the $0.39$-threshold operating-point
CI is reportable at the budgeted $n$.

The decision rule is pre-registered: \textbf{if Gate~0(b) shows the budget cannot
certify a useful NPV band, the abstention curve ships \emph{exploratory}, full
stop}, and the word ``guaranteed'' never appears anywhere in the paper. This is
not a kill criterion --- it relocates the headline weight of the wrapper onto the
welfare-asymmetric decision curve (Component~4), which is the more robust artifact
at small $n$. The arithmetic motivating this branch is sobering: certifying
NPV~$\geq 0.95$ would require on the order of $60$ zero-error abstained-in
negatives out of ${\sim}70$ total, which collapses the abstention rate toward zero
and yields a vacuous instrument. We therefore demote the claim \emph{on paper now}
rather than discover the shortfall after the vet's time is spent, and we report
the abstention curve as ``NPV with a one-sided $95\%$ lower bound at each
abstention rate'' --- a finite-sample statement, never a guarantee.
...
the binary spine rather than claim a \textsc{go} that a plausible correlation
...
interval, sensitivity and specificity at the $0.39$ cutoff will be estimated on
vet-confirmed labels only (the circularity firewall), and the agreement between
the model and the weak-labeling VLM will never be presented as validation. The
modal product --- the binary-plus-wrapper instrument with the two portable methods
and an inspected-not-validated graded layer --- is the outcome we plan for, and it
