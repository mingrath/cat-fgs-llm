...
This chapter follows the three-part structure of a conventional literature
review: a concise technical primer that fixes the vocabulary used throughout the
rest of this document (Section~\ref{sec:bg-primer}), a per-paper survey of the
feline and animal facial-pain literature that situates our work
(Section~\ref{sec:bg-related}), and a synthesis
(Section~\ref{sec:bg-synthesis}) that contrasts prior work against the headline
\emph{portable method} this project advances---a power-aware confound-attribution
protocol---together with its guarded VLM-as-AU-rater reliability check, and makes
the \enquote{not-a-replicate} argument explicit by enumerating the replication
...
the claimed novelty in the \emph{assembly} of the confound-attribution protocol
and its per-AU saliency-as-confound-evidence reading---not in the architecture,
not in any individual primitive, and not in the underlying audit statistics
themselves.
...
$>0.39$ is the published decision boundary indicating that analgesia should
be considered \cite{evangelista2019}. We inherit this $0.39$ point only as a
construct anchor: at the budgeted veterinary sample its per-class sensitivity and
specificity are pre-registered as non-reportable (Gate~0,
$n_{\text{pos}}{=}16$), and we never restate it as a validated operating point of
ours (see the limitations chapter and Section~\ref{sec:bg-synthesis}). The rubric wording adopted by the VLM
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[au, fill=red!8, right=1.4cm of sum] (dec)
      {Index $>0.39$\\$\Rightarrow$ consider\\analgesia};
...
DINOv2 ViT-S/14 encoder purely as a fixed feature extractor. We adopt the
register variant (\texttt{dinov2\_vits14\_reg}) as the field default because
registers suppress attention artifacts that hurt the dense, localized features
per-AU FGS scoring depends on (orbital, ear, muzzle sub-regions); it was
A/B-tested against the plain variant before locking ViT-S/14. The backbone is
...
empirical question this raises---posed by the guarded reliability check that
rides below this work's headline---is
whether such a frozen VLM can weak-label feline FGS action units at
human-rater agreement, measured against a veterinary anchor---a measurement that
reflects weak-labelling capability only when the anchor is independent and the
rubric handed to the VLM differs from the one the vet scored from, and otherwise
reflects rubric-following, so each run must state which it measures. That
measurement, not the act of prompting a VLM, is what the check delivers; the
rater is selected by its measured agreement rather than asserted to be adequate.
The check ships inspected-not-validated and is retained as kill-tree insurance,
not as a co-equal headline alongside the confound protocol.
...
the central artifact of the trustworthiness wrapper---which is cited supporting
plumbing rather than a headline contribution, but is the component that
distinguishes the wrapper from generic calibration---and the
...
$\{0,1,2\}$ without weights is a bug to avoid). The guarded reliability check of
this project---subordinate to the headline confound protocol---is exactly such a
per-AU quadratic weighted kappa between the VLM and
the veterinary anchor---shipped as a runnable check, with the number itself
pending the independent per-AU vet anchor (Gate 0)---gated on its confidence-interval \emph{lower bound}
...
$0.47$. Gating on the CI lower bound rather than the point estimate is
\emph{textbook clinimetrics}, not a novel increment: it is the standard
kappa-acceptance discipline and its sample-size planning counterpart established
for clinical ratings
\cite{tractenberg2010kappa,donner2010kappa,rotondi2012kappa,rotondi2018kappasize,simwright2005kappa};
the same ordinal-AU kappa/ICC-with-CI practice anchors the feline-pain domain
\cite{cheng2020fgs,evangelista2021fgs}. A rubric-paraphrase independence guard
(handing the VLM a reworded rubric so the check measures weak-labelling rather
than rubric-following) follows published policy-invariance practice for LLM
judges \cite{weng2026policy} and is likewise not claimed as novel. The
in-domain precedent for chatbot--expert FGS agreement reports plain
limits-of-agreement without a CI-lower-bound acceptance gate
\cite{chatbotfgs2025}; our only increment over it is the CI-lower-bound gate and
the rubric-independence guard, both of which are standard practice rather than
contributions. Separately, Krippendorff's alpha \cite{krippendorff2004} in its ordinal
...
\cite{xiao2021noise} demonstrates that models routinely
exploit such spurious foreground/background cues, and the complementary
foreground/background/attribute sensitivity analysis of
\cite{moayeri2022comprehensive} quantifies how much each contributes; together
they provide the primitive on which our
\emph{capture-condition confound audit} is built: a trivial classifier built on
...
context. The energy-based pointing game from the Score-CAM line
\cite{wang2020scorecam} quantifies saliency \emph{faithfulness}---how much of a
saliency map's energy falls inside the relevant region---and we re-read it
\emph{per AU} as saliency-as-confound-evidence: the per-AU energy-based pointing
game (EBPG) reported alongside the foreground/background gap (FGS-BG-Gap). We do
not claim either primitive as novel; the claimed novelty is their assembly into a
single audit and the per-AU EBPG-as-confound-evidence reading. We
distinguish three neighbours that touch parts of this surface but do not
assemble the same protocol. \textcite{lencioni2025segment} build a segment-based
explainability framework for animal affective computing, but own only the per-AU
saliency leg and frame it \emph{positively} (which regions explain the
prediction) rather than as one-directional confound evidence.
Background-context bias in affect vision--language models has been mitigated
\cite{context2025becki}, but in the \emph{human} emotion-recognition domain and
as a mitigation method, not a portable cross-corpus audit. Crucially this audit
is \emph{one-directional}: it is well powered to \emph{detect} confounding but
underpowered to rule it out, so its only honest conclusion is \enquote{no
confound detected at this power,} never \enquote{no confound}---a stance that
post-hoc explanations alone cannot license, since they can miss unknown spurious
correlations \cite{adebayo2022posthoc}. We instantiate this at a
\emph{pre-registered power}, borrowing equivalence/power-conditioned audit
hygiene from algorithmic-fairness testing
\cite{huanghooker2026fairness,singh2023samplesize}; the statistics themselves are
not ours. The
\emph{protocol}---a two-leg image-side audit, FGS-BG-Gap plus per-AU EBPG,
runnable on any future facial-pain corpus and validated today on planted positive
and negative controls---is the headline contribution of this work, distinct from
any verdict about a particular dataset. A third leg, a VLM judge-bias probe, is
fully coded but exercised on synthetic inputs only; applying it to a real VLM is
named here as future work and is not part of the present headline.
...
this project has not yet run experiments and reports no results of its own. This
is a structural state, not modesty: the working corpus is not yet forked (the
\texttt{data/} substrate is empty), the source is Roboflow binary pain/no-pain
only (no native $0$--$10$ FGS labels), and no independent per-AU veterinary
anchor exists, so the inherited $0.39$ point remains non-reportable at the
budgeted $n_{\text{pos}}{=}16$. All real-cat evaluation is future work; the
limitations chapter itemises these four facts.
...
\emph{not} a better detector. The headline is a single transportable protocol
that no prior feline-pain study assembled: a reusable, power-aware
capture-condition confound-attribution protocol with two image-side legs
(FGS-BG-Gap plus per-AU EBPG-as-confound-evidence), one-directional by
construction and validated today on planted controls. A coded-but-synthetic-only
VLM judge-bias leg is held as future work, not part of the present headline. Its claimed novelty is the
\emph{assembly}, the per-AU EBPG-as-confound-evidence reading, and the
instantiation of equivalence-style audit hygiene at a pre-registered
power---never any individual primitive
\cite{xiao2021noise,moayeri2022comprehensive,wang2020scorecam} and never the
underlying power/equivalence statistics, which we credit to their sources
\cite{huanghooker2026fairness,singh2023samplesize}; the one-directional
\enquote{no confound at this power} framing follows
\cite{adebayo2022posthoc}.
Subordinate to it, as a guarded, inspected-not-validated reliability check and
kill-tree insurance rather than a co-equal pillar, is the VLM-as-AU-rater per-AU
quadratic weighted kappa against a veterinary anchor, gated on the
confidence-interval lower bound---a gate that is textbook clinimetrics
\cite{tractenberg2010kappa,donner2010kappa,rotondi2012kappa,rotondi2018kappasize,simwright2005kappa},
with a published rubric-paraphrase guard \cite{weng2026policy}, neither claimed
as a novel increment. The DINOv2-plus-CORN engine carrying these methods is conceded as
...
curve as cited supporting plumbing, while the $0$--$10$ ordinal layer ships
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{3.0cm}p{3.1cm}p{3.3cm}cc@{}}
    ...
    \midrule
    \textbf{This work (headline)}
      & Power-aware confound-attribution protocol
      & \emph{FGS-BG-Gap + per-AU EBPG (one-directional)}
      & Audit & \textbf{Yes} \\
    \textbf{This work (guarded check)}
      & VLM-as-AU-rater, vs.-vet kappa
      & \emph{CI-lower-bound-gated per-AU QWK (pending vet anchor)}
      & AU rating & Yes (guarded check) \\
    \textbf{This work (cited plumbing)}
      & Welfare-asymmetric DCA + LB-abstention
      & \emph{net benefit (range); one-sided 95\% NPV LB}
      & Binary wrapper & Yes (cited plumbing) \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Binary-detector replication.} Producing a backbone-swapped
    ...
    portable confound-attribution protocol, not the detector, is the headline.
...
metrics} critique: the headline surviving claim is the reusable, power-aware
confound-attribution protocol (validated today on planted controls), with the
VLM-vs-vet kappa as a guarded reliability check below it (whose result is pending
the independent per-AU vet anchor) and kept as kill-tree insurance---both
portable to the next dataset, riding on an explicitly conceded engine.
