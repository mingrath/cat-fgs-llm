...
contributions of this work live entirely in the trustworthiness wrapper and the
two portable methods built on top of the engine's outputs (the VLM-as-AU-rater
$\kappa$ protocol and the capture-condition confound-attribution protocol),
which are developed in later chapters. Everything specified here exists only to
...
$384$, patch size $14$, forward-only. We freeze it because the supervised set is
...
single-machine, single-vet regime. Its outputs feed the trustworthiness wrapper
and the two portable methods that carry this work's contribution. Nothing here
